...
PROC COMPARE - i.e. human-readable reporting on the difference between
two dataframes.
"""
...
class Compare:
    ...
    def sample_mismatch(self, column, sample_count=60, for_display=False):
        ...
        sample_count : int, optional
            The number of sample records to return.  Defaults to 60.
    ...
    def report(self, sample_count=60):
        ...
        if self.df1_unq_rows.shape[0] > 0:
            report += "Sample Rows Only in {} (First 60 Columns)\n".format(self.df1_name)
            report += "---------------------------------------{}\n".format("-" * len(self.df1_name))
            report += "\n"
            columns = self.df1_unq_rows.columns[:60]
        ...
        if self.df2_unq_rows.shape[0] > 0:
            ...
            report += "\n"
            columns = self.df2_unq_rows.columns[:60]
...
def render(filename, *fields):
    ...
    ----------
    filename : str
        The file that contains the template.  Will use automagic to prepend the
